...
public class Fox extends Animal  {
    Fox(World organism_world, Container world_organisms, Organism organisms_array[][], boolean born, int parent_pos_x, int parent_pos_y) {
        ...
        if (born == false) { // Losowanie pozycji i sprawdzanie czy nie jest juz zajeta dla nie urodzonego organizmu
            while (true) {
                ...
                if (organisms_array[posX][posY] == null) {
                   setOrganism(3, 7, 1, "Fox", posX, posY, organism_world);
    ...
    Fox(World organism_world,Container world_organisms, Organism organisms_array[][]){
        boolean born = false;
        int parent_pos_x=0 , parent_pos_y=0;
        new Fox(organism_world,world_organisms,organisms_array,born,parent_pos_x,parent_pos_y);
    }
